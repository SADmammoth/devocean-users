 ...
 * Example usage:
 * ```
 *     return res.notUnique();
 *     // -or-
 *     return res.notUnique(optionalData);
 * ```
 *
 * Or with actions2:
 * ```
...
module.exports = function notUnique(optionalData) {
  ...
  if (optionalData === undefined) {
    sails.log.info('Ran custom response: res.notUnique()');
  ...
  // except in production use res.sendStatus().
    ...
    sails.log.info(
      'Custom response `res.notUnique()` called with an Error:',
      optionalData,
    );
